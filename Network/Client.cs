...
/// </summary>
public class Client : Singleton<Client>, IInitializeable
{
    ...
    */
    ...
    /// <summary>
    /// Shot when the player is disconnected.
    /// </summary>
    ...
    public void Login(string user, string password)
    {
        ...
        message.Write((ushort)AccessLevel.Player);
        message.Write(user);
        netclient.Connect(Server.Master.IPAddress, Server.Master.Port, message);
    ...
    {
        NetOutgoingMessage message = netclient.CreateMessage();
        message.Write((int)PlayerAction.GetServers);
	...
	public void RequestRedirect(uint characterId, int realmId)
	{
		...
		message.Write((ushort)PlayerAction.RequestRedirect);
    ...
    public void Connect(Server server, string token, string username, uint characterId)
    {
		...
		Initialize();
		NetOutgoingMessage hailMessage = netclient.CreateMessage();
		hailMessage.Write((ushort)AccessLevel.Player);
		hailMessage.Write(username);
		hailMessage.Write((uint)characterId);
		hailMessage.Write(token);
		netclient.Connect(server.IPAddress, server.Port, hailMessage);
    }
				...
				try
				{
					switch (incoming.MessageType)
					{
						case NetIncomingMessageType.Data:
							PlayerAction action = (PlayerAction)incoming.ReadUInt16();
							switch (action)
							{
								case PlayerAction.GetServers:
                                ...
                                case PlayerAction.GetAccount:
                                    ...
                                    break;
								case PlayerAction.RequestRedirect:
									OnReceiveRedirect();
									...
									break;
                                default:
                                    Debug.Log("Received unknown message type: " + (PlayerAction)action);
                                    break;
							}
							break;
                        case NetIncomingMessageType.StatusChanged:
                            NetConnectionStatus status = (NetConnectionStatus)incoming.ReadByte();
							Debug.Log("NetConnectionStatus changed: " + status.ToString());
                            switch (status)
                            {
